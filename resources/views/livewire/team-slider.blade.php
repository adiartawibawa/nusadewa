<div>
    <div x-data="{
        ...
        scrollToSlide() {
            const slide = $refs.track.children[this.index];
            if (slide) slide.scrollIntoView({ behavior: 'smooth', inline: 'start' });
        }
    }" x-init="startAutoplay()" @mouseenter="stopAutoplay()" @mouseleave="startAutoplay()"
        class="relative h-[80vh] min-h-[600px] bg-transparent overflow-hidden">
        ...
        <button @click="next"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" stroke-width="1.5"
            ...
            @foreach ($teamMembers as $index => $member)
                <div class="flex-shrink-0 w-full snap-start">
                    <div class="flex flex-col items-center h-full gap-10 px-8 py-10 mx-auto max-w-7xl md:flex-row">
                        ...
                        <!-- Bio Content (scrollable only on hover) -->
...
@push('styles')
    <style>
        /* Default: scrollbar tetap aktif tapi transparan (tidak terlihat) */
